...
try:
    ...
    name_col = next(
        (c for c in df.columns if c.lower() in ["id", "track name", "name"]), None
    )
    ...
    elev_col = next(
        (c for c in df.columns if c in ["elevation", "ellipsoidal height", "height"]),
        None,
    )
    ...
    if time_col:
        ...
        df["avg_time_end"] = (df[time_col] + timedelta(seconds=4)).apply(
            format_utc_with_ms
        )
    ...
    mapped = pd.DataFrame(
        {
            ...
            "Averaging start": df["avg_time_start"]
            if time_col
            else pd.Series([""] * n_rows),
            "Averaging end": df["avg_time_end"]
            if time_col
            else pd.Series([""] * n_rows),
            ...
            "Antenna height": df[inst_height_col]
            if inst_height_col
            else pd.Series([""] * n_rows),
            "Antenna height units": pd.Series(["m"] * n_rows),
            "PDOP": df[pdop_col]
            if pdop_col
            else pd.Series([""] * n_rows),  # <-- PDOP mapping
        }
    )
